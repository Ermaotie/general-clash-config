...
async function devices(env, origin) {
  const rows = await env.DB.prepare('SELECT id,name,token_encrypted,revoked_at,created_at FROM devices ORDER BY created_at DESC').all()
  return Promise.all((rows.results || []).map(async row => ({
    id: row.id,
    name: row.name,
    createdAt: row.created_at,
    revokedAt: row.revoked_at,
    subscription: row.token_encrypted && !row.revoked_at ? `${origin}/sub/${await open(row.token_encrypted, env)}` : null,
    legacy: !row.token_encrypted
  })))
}
...
function pageWithDevices() {
  ...
  const runtime = `<script>window.loadDevices=async function(){const r=await fetch('/api/manage/devices',{headers:headers()});if(!r.ok)return msg('读取设备失败：请检查管理令牌','error');const data=await r.json();const list=$('device-list');list.replaceChildren();if(!data.devices.length){list.textContent='尚未生成设备订阅。';return}for(const d of data.devices){const row=document.createElement('div');row.className='device-row';const meta=document.createElement('div');const name=document.createElement('b');name.textContent=d.name;const state=document.createElement('span');state.className='hint';state.textContent=d.revokedAt?'已撤销':d.legacy?'旧设备':'正常';meta.append(name,document.createElement('br'),state);row.append(meta);if(d.subscription){const input=document.createElement('input');input.readOnly=true;input.value=d.subscription;row.append(input)}else{const note=document.createElement('span');note.className='hint';note.textContent=d.legacy?'旧链接无法恢复，请新建替换。':'已撤销';row.append(note)}const actions=document.createElement('div');actions.className='row';if(d.subscription){const copy=document.createElement('button');copy.textContent='复制';copy.onclick=async()=>{await navigator.clipboard.writeText(d.subscription);msg('设备订阅已复制','success')};actions.append(copy)}if(!d.revokedAt){const revoke=document.createElement('button');revoke.className='danger';revoke.textContent='撤销';revoke.onclick=async()=>{if(!confirm('撤销后该设备订阅将立即失效，确定继续吗？'))return;const result=await fetch('/api/manage/devices/'+d.id,{method:'DELETE',headers:headers()});if(!result.ok)return msg('撤销失败：请检查管理令牌','error');await window.loadDevices();msg('设备订阅已撤销','success')};actions.append(revoke)}row.append(actions);list.append(row)}};</script>`
  const cardRuntime = `<script>const deviceStyle=document.createElement('style');deviceStyle.textContent='#device-list{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:12px}.device-card{padding:15px;border:1px solid #e2e8f0;border-radius:14px;background:#f8fafc}.device-card h3{margin:0 0 6px;font-size:15px}.device-card p{margin:0;color:#64748b;font-size:12px}.device-card .row{margin-top:13px}.device-card .danger{align-self:auto}';document.head.append(deviceStyle);window.loadDevices=async function(){const r=await fetch('/api/manage/devices',{headers:headers()});if(!r.ok)return msg('读取设备失败：请检查管理令牌','error');const data=await r.json();const active=data.devices.filter(d=>!d.revokedAt);const list=$('device-list');list.replaceChildren();if(!active.length){list.textContent='尚未生成设备订阅。';return}for(const d of active){const card=document.createElement('article');card.className='device-card';const title=document.createElement('h3');title.textContent=d.name;const state=document.createElement('p');state.textContent=d.legacy?'旧设备：原链接不可恢复':'正常';card.append(title,state);const actions=document.createElement('div');actions.className='row';if(d.subscription){const copy=document.createElement('button');copy.textContent='复制订阅链接';copy.onclick=async()=>{await navigator.clipboard.writeText(d.subscription);msg('设备订阅已复制','success')};actions.append(copy)}const revoke=document.createElement('button');revoke.className='danger';revoke.textContent='撤销';revoke.onclick=async()=>{if(!confirm('撤销后该设备订阅将立即失效，确定继续吗？'))return;const result=await fetch('/api/manage/devices/'+d.id,{method:'DELETE',headers:headers()});if(!result.ok)return msg('撤销失败：请检查管理令牌','error');await window.loadDevices();msg('设备订阅已撤销','success')};actions.append(revoke);card.append(actions);list.append(card)}};const originalLoadSettings=window.loadSettings;window.loadSettings=async function(){await originalLoadSettings();await window.loadDevices()};</script>`
  return page().replace('<div id="message"', `${toast}${manager}<div id="message"`).replace('</html>', `${runtime}${cardRuntime}</html>`)
}
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === '/api/devices' && request.method === 'POST') {
      ...
      await env.DB.prepare('INSERT INTO devices (id,name,token_hash,token_encrypted,created_at) VALUES (?,?,?,?,?)').bind(id, String(input.name || '设备').slice(0, 64), await hash(token), await seal(token, env), new Date().toISOString()).run()
      return Response.json({ subscription: `${url.origin}/sub/${token}` })
    }
    if (url.pathname.startsWith('/api/manage/devices/') && request.method === 'DELETE') {
      if (!authorized(request, env)) return new Response('Unauthorized', { status: 401 })
      const id = url.pathname.slice('/api/manage/devices/'.length)
      if (!/^[0-9a-f-]{36}$/i.test(id)) return new Response('Not found', { status: 404 })
      await env.DB.prepare('UPDATE devices SET revoked_at=? WHERE id=? AND revoked_at IS NULL').bind(new Date().toISOString(), id).run()
